"""Rigid motion estimation and tracking from navigator images.

A navigator resolves six degrees of freedom out of images that are each only
two-dimensional: one plane sees the motion in its own plane, and planes that
between them span the volume see all of it. Planes are registered against the
first navigator acquired, solved together for the pose that explains all of
them, and carried across the scan by an extended Kalman filter.

Registration runs under :func:`mrmotion.single_threaded`. Prospective
correction shares a machine with the reconstruction it steers, and owes it a
pose before the next acquisition rather than a pose as fast as possible.
"""
...
from ._threads import single_threaded
...
class RigidRegistration:
    ...
    .. plot::
       ...
       estimate = mm.RigidRegistration(iterations=100)(truth, moved)
    ...
    def estimate(
        ...
        *,
        ...
        with single_threaded(self.threads or 1):
...
class RigidMotionEKF:
    ...
    registration is smoothed by what came before it::
        ...
        tracker = mm.RigidMotionEKF(mm.RigidRegistration(), dimension=3)
    ...
    .. plot::
       ...
       tracker = mm.RigidMotionEKF(
           mm.RigidRegistration(),
       ...
       for pose in measured:
           ...
           filtered.append(
               tracker.update(
                   mm.RigidMotionEstimate(parameters=pose, center=np.zeros(3))
               ).parameters
           )
...
def _pose_from_planes(
    ...
    """Solve for the one pose that best explains what every plane measured.
...
def _project_onto_plane(
    ...
    """Predict what one plane would measure if the object were at ``pose``."""
