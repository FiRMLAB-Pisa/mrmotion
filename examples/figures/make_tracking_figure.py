...
plane = mm.reconstruct_navigator(
    acquire(brain)[None, None], traj[None], (GRID, GRID), density=dens[None]
)[0]
...
def navigator(translation):
    ...
    for u, v in AXES:
        ...
        out.append(
            shift(
                anat,
                (np.dot(translation, u) / SPACING, np.dot(translation, v) / SPACING),
                order=3,
            )
        )
    return out
...
truth = np.array(
    [np.array([2.0, -1.5, 3.0]) * np.sin(s / 5) for s in range(1, NAV + 1)]
)
...
def run(process_noise):
    ...
    return np.asarray(
        [
            t.track(
                navigator(p + rng.normal(0, 0.4, 3)), AXES, dt=DT, spacing=SPACING
            ).translation
            for p in truth
        ]
    )
...
print(
    f"measured rms {np.sqrt(((measured - truth) ** 2).mean()):.3f} mm, "
    f"filtered rms {np.sqrt(((filtered - truth) ** 2).mean()):.3f} mm"
)
